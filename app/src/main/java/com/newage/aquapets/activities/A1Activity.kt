...
import com.newage.aquapets.customacitivityresultcontracts.CreateTankActivityResultContract
...
import com.newage.aquapets.helpers.parcelable
import com.newage.aquapets.models.GalleryInfo
...
class A1Activity : AppCompatActivity(), NavigationView.OnNavigationItemSelectedListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (storedVersionCode > 0 && storedVersionCode != currentVersionCode) {
            builder.setTitle(resources.getString(R.string.Attention))
                ...
                .setNeutralButton(resources.getString(R.string.OK)) { dialog, _ -> dialog.dismiss() }
                .create().show()
        }
    ...
    private val resultContract = registerForActivityResult(CreateTankActivityResultContract()){

         tankDetails ->
        run {
            tankDetails?.let {
                a1ViewModel.getTankDetailsArrayList()!!.add(it)
    ...
    override fun onOptionsItemSelected(item: MenuItem): Boolean {
        ...
        if (item.itemId == R.id.addDetails) {
           /* val intent = Intent(this, CreateTankActivity::class.java)
            ...
            resultContract.launch(TANK_DETAILS_CREATION)
    ...
    private fun startMapsActivty() {
        if (authService.user == null) {
            ...
            builder.setTitle(resources.getString(R.string.Attention))
                ...
                .setNeutralButton(resources.getString(R.string.OK)) { dialogInterface, _ -> dialogInterface.dismiss() }
                .create().show()
        } else {
            // String userID = "ma";
            validateAndProceed()
        ...
        if (resultCode == Activity.RESULT_OK) {
            ...
            if (requestCode == TANK_DETAILS_CREATION) {
               /* val tanksDetails = data!!.getParcelableExtra<TanksDetails>("TankItemsObject")
                ...
                val tanksDetails =  data!!.parcelable("TankItemsObject",TankDetails::class.java)

                val position = data.getIntExtra("Position", -1)
                a1ViewModel.getTankDetailsArrayList()!![position] = tanksDetails!!
                a1ViewModel.calcCumExpense(position)
                Handler(Looper.getMainLooper()).post { tanksSectionsPagerAdapter!!.notifyItemChanged(position) }
                ...
                Handler(Looper.getMainLooper()).post { tanksSectionsPagerAdapter!!.notifyDataSetChanged() }
